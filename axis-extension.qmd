...
For magnitude ratings, a likelihood ratio test revealed a significant interaction between axis upper bound and denominator information: $\chi^2$(`r in_paren(e2_mag.df)`) = `r printnum(e2_mag.LR)`, p `r printp(e2_mag.p, add_equals = TRUE)`, partial $\eta^2$ = `r printnum(e2_mag.eta.axis_denominator)`.

Pairwise comparisons reveal that charts with extended and truncated axes were rated differently when the denominator was present, replicating the effect from E1, and when the denominator was absent. Therefore, the interaction indicates that the degree of difference in magnitude ratings was affected by whether the denominator was present or absent.
...
get_contrasts <- function(emm, fctor){
  
  contrast(emm, "consec", simple = "each", combine = TRUE, adjust = "sidak") %>%
  as_tibble()  %>%
  filter({{ fctor }} != ".")
