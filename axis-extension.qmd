...
### Magnitude Ratings and Literacy
...
```{r}
#| label: e1-mag-l-anova

anova_results(e1_mag_l, e1_mag)
```

Including data visualisation literacy as an additional fixed effect did not improve the explanation of variance in magnitude ratings: $\chi^2$(`r in_paren(e1_mag_l.Df)`) = `r printnum(e1_mag_l.Chi)`, p `r printp(e1_mag_l.p, add_equals = TRUE)`.
...
e2 %>%
  ...
  geom_jitter(width = 0,
              height = 0.2,
              alpha = 0.1) +
  scale_y_discrete(limits = c("full", "trunc"),
                   labels = c("Extended", "Truncated")) + 
...
emmip(e2_mag@model, denominator ~ axis, CIs = T) +
  ...
  labs(x = "Axis Upper Bound",
       ...
       title = "Experiment 2 - Estimated Marginal Means") + 
