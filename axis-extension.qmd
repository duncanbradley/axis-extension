...
The final sample consisted of 150 participants (\`r printnum(gender_E1\$M)\`% male, \`r printnum(gender_E1\$F)\`% female, \`r printnum(gender_E1\$NB)\`% non-binary). Mean age was \`r printnum(age_E1\$mean)\` (\*SD\* = \`r printnum(age_E1\$sd)\`). The mean data visualisation literacy score was \`r printnum(literacy_E1\$mean)\` (\*SD\* = \`r printnum(literacy_E1\$sd)\`), out of a maximum of 30.

Average completion time was `r printnum(e1 %>% pull(total_duration) %>% mean())` minutes. Participants with accepted submissions received £3.50.
...
Differences in filler trials.

We added the word \'surveyed\' or \'assessed\' to \[x\] accompanying sentences in items from E1, that were unclear in the absence of a denominator.

### Procedure

The procedure was identical to Experiment 1, except for the addition of a confidence rating...

Average completion time was `r printnum(e2 %>% pull(total_duration) %>% mean())` minutes.
...
The final sample consisted of 200 participants (\`r printnum(gender_E1\$M)\`% male, \`r printnum(gender_E1\$F)\`% female, \`r printnum(gender_E1\$NB)\`% non-binary). Mean age was \`r printnum(age_E1\$mean)\` (\*SD\* = \`r printnum(age_E1\$sd)\`). The mean data visualisation literacy score was \`r printnum(literacy_E1\$mean)\` (\*SD\* = \`r printnum(literacy_E1\$sd)\`), out of a maximum of 30.
