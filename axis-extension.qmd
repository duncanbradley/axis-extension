---
title: "Bar Charts, Magnitude Judgements, and Denominator Information"
format: html
editor: visual
...
https://osf.io/e9j43 https://osf.io/hx9y4
...
We generated bar charts in R using {ggplot2} (version 4.1.2), {tidyverse} (version 1.3.1) and {ggh4x} (version 0.2.1). The two versions of each chart displayed the same five values, but employed different y-axis limits. Denominator values (400, 500, or 600) were used to generate datasets: data were sampled from normal distribution with a mean equal to either 20% or 40% of a given denominator value, and a standard deviation equal to 1% of the denominator value.
...
Accounting for differences in data visualisation literacy did not change the significant effect of Axis Limit: F(`r printnum(e1_mag_l_cond_NumDF)`, `r printnum(e1_mag_l_cond_DenDF)`) = `r printnum(e1_mag_l_cond_F.value)`, p `r printp(e1_mag_l_cond_p, add_equals = T)`, partial $\eta^2$ = `r printnum(e1_mag_l_cond_Eta2_partial)`.
...
`r printnum(get_anomalies(e2), digits = 0)`% of datasets used were re-generated to ensure that highest gridline of a default axis did not exceed the highest plotted value.
...
Crucially, there was also a significant interaction between Axis Limit and denominator presence: F(`r printnum(e2_mag_axis_denominator_NumDF)`, `r printnum(e2_mag_axis_denominator_DenDF)`) = `r printnum(e2_mag_axis_denominator_F.value)`, p `r printp(e2_mag_axis_denominator_p, add_equals = T)`, partial $\eta^2$ = `r printnum(e2_mag_axis_denominator_Eta2_partial)`. @fig-e2-mag-int plots this interaction.
