...
p_label <- ggplot(tibble()) + geom_point(aes(x = 1, y = 1.5), alpha = 0.2, size = 2) + xlim(1, 2) + ylim(1, 2) + coord_fixed(ratio=0.1) + annotate("text", label = " = one rating", x = 1.2, y = 1.5) + theme_void()
```

```{r}
#| label: fig-e1-mag
#| include: true
p_label / p_data + plot_annotation(
  title = "Experiment 1",
  subtitle = "Magnitude Ratings")
```
